...
class SFace:
    ...
    def match(self, image1, face1, image2, face2):
        ...
        if self._disType == 0: # COSINE
            ...
            return (1, cosine_score) if cosine_score >= self._threshold_cosine else (0, cosine_score)
        else: # NORM_L2
            norml2_distance = self._model.match(feature1, feature2, self._disType)
            return (1, norml2_distance) if norml2_distance <= self._threshold_norml2 else (0, norml2_distance)
    
    def compareTwoFeatures(self, feature1, feature2):
